...
# Load Data
model_data_load = fread("./data/Alzheimers_Disease_cleandata_final.csv")
model_data = as.data.frame(model_data_load)
...
print("--- Training Random Forest on Not Batch-Corrected Data ---")
...
# Variable Importance
# Save the base plot
png(filename = "RF_VarImp.png", width = 800, height = 600)
varImpPlot(rf_model, main = "Variable Importance (Not Batch Corrected)")
dev.off()
...
p_imp = ggplot(top_20_imp, aes(x = reorder(Variable, MeanDecreaseAccuracy), y = MeanDecreaseAccuracy)) +
  ...
  theme_minimal() +
  labs(
    title = "Top 20 Variable Importance (Not Batch Corrected)",
...
p_roc = ggroc(roc_obj, color = "darkgreen", size = 1.2) + # Changed color to distinguish from non-corrected
  theme_minimal() +
  labs(
    title = paste("ROC Curve (Not Batch Corrected) | AUC =", round(auc(roc_obj), 3)),
...
p_err = ggplot(err_long, aes(x = Trees, y = Error, color = ErrorType)) +
  ...
  theme_bw() +
  labs(
    title = "OOB and Class Error Rate vs. Trees (Not Batch Corrected)",
    y = "Error Rate",
    color = "Metric"
  )
...
gene_expression = as.matrix(model_data_load[, !metadata_cols, with = FALSE])
gene_expression_t = t(gene_expression) 

# Create the Design Matrix (preserving the biological condition of interest)
design_mat = model.matrix(~ model_data_load$Alzheimers_Disease)
...
batch_corrected_mat = removeBatchEffect(
  x = gene_expression_t,
  batch = model_data_load$batch_id,
  design = design_mat
)
...
batch_corrected_df$Alzheimers_Disease = model_data_load$Alzheimers_Disease
